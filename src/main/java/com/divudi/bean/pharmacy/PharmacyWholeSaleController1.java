...
@Named
@SessionScoped
public class PharmacyWholeSaleController1 implements Serializable {

    /**
     * Creates a new instance of PharmacySaleController
     */
    public PharmacyWholeSaleController1() {
    }
...
//        createPaymentSchemeItems();
    ...
    public List<Stock> completeAvailableStocks(String qry) {
        ...
        if (qry.length() > 4) {
            sql = "select i from Stock i where i.stock >:s and i.department=:d and ((i.itemBatch.item.name) like :n or upper(i.itemBatch.item.code) like :n or upper(i.itemBatch.item.barcode) like :n )  order by i.itemBatch.item.name, i.itemBatch.dateOfExpire";
        } else {
            sql = "select i from Stock i where i.stock >:s and i.department=:d and ((i.itemBatch.item.name) like :n or upper(i.itemBatch.item.code) like :n)  order by i.itemBatch.item.name, i.itemBatch.dateOfExpire";
        }
    ...
    public List<Item> completeRetailSaleItemsWithoutStocks(String qry) {
        ...
        if (qry.length() > 4) {
            sql = "select i from Amp i "
                    ...
                    + "((i.name) like :n or upper(i.code) like :n  or upper(i.barcode) like :n  or upper(i.vmp.name) like :n) and "
                    + "i.id not in(select ibs.itemBatch.item.id from Stock ibs where ibs.stock >:s and ibs.department=:d and ((ibs.itemBatch.item.name) like :n or upper(ibs.itemBatch.item.code) like :n  or upper(ibs.itemBatch.item.barcode) like :n  or upper(ibs.itemBatch.item.vmp.name) like :n )  ) "
                    + "order by i.name ";
            ...
            sql = "select i from Amp i "
                    + "where i.retired=false and "
                    + "((i.name) like :n or upper(i.code) like :n or upper(i.vmp.name) like :n) and "
                    + "i.id not in(select ibs.itemBatch.item.id from Stock ibs where ibs.stock >:s and ibs.department=:d and ((ibs.itemBatch.item.name) like :n or upper(ibs.itemBatch.item.code) like :n or upper(ibs.itemBatch.item.vmp.name) like :n )  ) "
                    + "order by i.name ";
