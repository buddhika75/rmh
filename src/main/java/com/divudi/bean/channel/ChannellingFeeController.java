    ...
    public List<Staff> completeStaff(String query) {
        ...
        if (query == null) {
            ...
            if (getFee().getSpeciality() == null) {
                sql = "select p from Staff p where p.retired=false and ((p.person.name) like :qry or upper(p.code) like :qry ) order by p.person.name";
            } else {
                sql = "select p from Staff p where p.speciality=:spe and p.retired=false and ((p.person.name) like :qry or  upper(p.code) like :qry) order by p.person.name";
