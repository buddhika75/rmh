    ...
    public List<Staff> completeStaff(String query) {
        ...
        if (query == null) {
            ...
            if (getCurrentFee().getSpeciality() == null) {
                ...
                sql = "select p from Staff p where p.speciality.id=" + getCurrentFee().getSpeciality().getId() + " and p.retired=false and ((p.person.name) like '%" + query.toUpperCase() + "%'or  upper(p.code) like '%" + query.toUpperCase() + "%' ) order by p.person.name";
            }
