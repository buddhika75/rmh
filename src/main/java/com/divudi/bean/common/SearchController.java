...
//    
    ...
    private void checkLabReportsApproved(List<Bill> bills) {
        for (Bill b : bills) {
            ...
            m.put("b", b);
            List<PatientReport> list = getPatientReportFacade().findBySQL(sql, m);
            if (!list.isEmpty()) {
...
//    
    ...
    private String fillUserPatientReport(boolean web) {
        ...
        jpql = " select pr from PatientInvestigation pr where pr.retired=false and "
                ...
                + " ((pr.billItem.bill.insId)=:bn or upper(pr.billItem.bill.deptId)=:bn) "
                + " order by pr.id desc ";
        if (web) {
            m.put("pn", getSessionController().getPhoneNo());
        } else {
...
//    
    ...
    public void channelPaymentBills() {
        ...
        if (getSearchKeyword().getBillNo() != null && !getSearchKeyword().getBillNo().trim().equals("")) {
            sql += " and  ((bi.paidForBillFee.bill.insId) like :billNo or upper(bi.paidForBillFee.bill.deptId) like :billNo )";
            m.put("billNo", "%" + getSearchKeyword().getBillNo().trim().toUpperCase() + "%");
        }
...
//
//                } else {
//                    sql = "select b from BilledBill b where b.retired=false and b.billType=:type and b.createdAt between :fromDate and :toDate and ((b.staff.person.name) like '%" + txtSearch.toUpperCase() + "%'  or upper(b.staff.person.phone) like '%" + txtSearch.toUpperCase() + "%'  or upper(b.insId) like '%" + txtSearch.toUpperCase() + "%') order by b.createdAt desc  ";
//                    temMap.put("toDate", getToDate());
//                    temMap.put("fromDate", getFromDate());
//                    temMap.put("type", BillType.PaymentBill);
//                    bills = getBillFacade().findBySQL(sql, temMap, TemporalType.TIMESTAMP, 100);
